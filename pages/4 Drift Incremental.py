...
st.write("""
         ### Simulation : 
""")
df = pd.read_csv("data/drybean_incremental.csv")

with st.popover(":gear: Modifier les paramètres"):
    st.write("""
     :gear: Modifier les paramètres de la simulation 
     """)
    window_size = st.number_input('Introduire la taille de la fenêtre', min_value=1, value=100, placeholder="Taille de la fenêtre")
    ...
    alert_thold=st.number_input('Introduire le seuil d\'alerte', min_value=0.1, value=3.4, placeholder="Seuil d'alerte")
    detect_thold=st.number_input('Introduire le seuil de détection', min_value=0.1, value=3.7, placeholder="Seuil de détection")
...
if button:
    ...
    for i in range(window_size, len(df)+1):
        ...
        chart.line_chart(df['ConvexArea'].iloc[:i])

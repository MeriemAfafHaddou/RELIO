...
st.set_page_config(
    page_title="Simulation : Recurrent Drift",
...
st.write("""
# Simulation of a recurrent drift
""")

with st.expander(
        ":blue[:question: What is a recurrent drift?]", expanded=False):
    st.write("""
        Recurrent concept drift refers to data changes that reappear
        after some time, without necessarily following a periodic
...
with btn1:
    with st.popover(":gear: Modify parameters"):
        ...
        window_size = st.number_input(
            "Enter the window size", min_value=1, value=20,
...
if button:
    ...
    st.divider()
    st.write(f"""
    ##### 	📉 Impact of recurrent drift - {METRIC_NAME} Evolution:
    """)

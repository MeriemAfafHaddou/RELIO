...
st.set_page_config(
    page_title="Simulation - Sudden Drift",
...
""")
with st.expander(":blue[:question: What is a sudden drift?]", expanded=False):
    st.write("""
        Sudden concept drift occurs when the model built on a certain
        concept becomes obsolete because the data distribution changes
...
with button1:
    with st.popover(":gear: Modify parameters"):
        ...
        window_size = st.number_input(
            "Enter the window size", min_value=1, value=50,
...
if button:
    ...
    st.divider()
    st.write(f"""
    ##### 	📉 Impact of sudden drift - {METRIC_NAME} Évolution :
    """)
